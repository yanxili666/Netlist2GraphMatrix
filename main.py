...
work_dir = os.getcwd()
circuits_path = "E:/PythonProject/OPAMP-Generator-main/OPAMP-Generator-main/testbench/mapped_circuits/"
...
for circuit in all_circuits:
    ...
    for line in netlist.readlines():
        ...
        if matchR:
            if ((matchR.groups()[1] == 'net_vo') or (matchR.groups()[1] == 'net_vin')) and (matchR.groups()[2] == '0'):
                pass
            else:
            ...
            if ((matchC.groups()[1] == 'net_vo') or (matchC.groups()[1] == 'net_vin')) and (matchC.groups()[2] == '0'):
                pass
            else:  
    ...
    #totalPortDict, deviceDict = removeIO(totalPortDict, deviceDict)

    # construct the internal structures of all G
    for device in deviceList:
        #print(device, deviceDict[device], '\n')
        if device[0:1] == 'G':
    ...
    for port in totalPortDict.keys():
        ...
        if port == 'net_vin':
            for dev in portDict['next']:            
                if dev[0:1] == 'R':
                    node2Node(graphMatrix, vinIndex, (rIndex+deviceDict[dev]['Idx']))
                elif dev[0:1] == 'C':
                    node2Node(graphMatrix, vinIndex, (cIndex+deviceDict[dev]['Idx']))
                elif dev[0:1] == 'G':
                    node2G(graphMatrix, deviceDict[dev], vinIndex, mosIndex, deviceDict[dev]['Inverse'])         
        elif port == '0':
            for dev in portDict['last']:
                if dev[0:1] == 'R':
                    node2Node(graphMatrix, (rIndex+deviceDict[dev]['Idx']), groundIndex)
                elif dev[0:1] == 'C':
                    node2Node(graphMatrix, (cIndex+deviceDict[dev]['Idx']), groundIndex)
                elif dev[0:1] == 'G':
                    g2Node(graphMatrix, deviceDict[dev], groundIndex, mosIndex, deviceDict[dev]['Inverse'])
        elif port == 'net_vo':
            for dev in portDict['next']:               
                if dev[0:1] == 'R':
                    node2Node(graphMatrix, voIndex, (rIndex+deviceDict[dev]['Idx']))
                elif dev[0:1] == 'C':
                    node2Node(graphMatrix, voIndex, (cIndex+deviceDict[dev]['Idx']))
                elif dev[0:1] == 'G':
                    node2G(graphMatrix, deviceDict[dev], voIndex, mosIndex, deviceDict[dev]['Inverse']) 
            for dev in portDict['last']:
                if dev[0:1] == 'R':
                    node2Node(graphMatrix, (rIndex+deviceDict[dev]['Idx']), voIndex)
                elif dev[0:1] == 'C':
                    node2Node(graphMatrix, (cIndex+deviceDict[dev]['Idx']), voIndex)
                elif dev[0:1] == 'G':
            ...
            for dev1 in portDict['last']:
                for dev2 in portDict['next']:
                    if (dev1[0:1] == 'R') or (dev1[0:1] == 'C'):
                        idx1 = deviceDict[dev1]['Idx']
                    if (dev2[0:1] == 'R') or (dev2[0:1] == 'C'):
                        idx2 = deviceDict[dev2]['Idx']
                    
                    if (dev1[0:1] == 'R') and (dev2[0:1] == 'R'):
                        node2Node(graphMatrix, rIndex+idx1, rIndex+idx2)
                    elif (dev1[0:1] == 'R') and (dev2[0:1] == 'C'):
                        node2Node(graphMatrix, rIndex+idx1, cIndex+idx2)
                    elif (dev1[0:1] == 'R') and (dev2[0:1] == 'G'):
                        node2G(graphMatrix, deviceDict[dev2], rIndex+idx1, mosIndex, deviceDict[dev2]['Inverse'])
                    elif (dev1[0:1] == 'C') and (dev2[0:1] == 'R'):
                        node2Node(graphMatrix, cIndex+idx1, rIndex+idx2)
                    elif (dev1[0:1] == 'C') and (dev2[0:1] == 'C'):
                        node2Node(graphMatrix, cIndex+idx1, cIndex+idx2)
                    elif (dev1[0:1] == 'C') and (dev2[0:1] == 'G'):
                        node2G(graphMatrix, deviceDict[dev2], cIndex+idx1, mosIndex, deviceDict[dev2]['Inverse'])
                    elif (dev1[0:1] == 'G') and (dev2[0:1] == 'R'):
                        g2Node(graphMatrix, deviceDict[dev1], rIndex+idx2, mosIndex, deviceDict[dev1]['Inverse'])
                    elif (dev1[0:1] == 'G') and (dev2[0:1] == 'C'):
                        g2Node(graphMatrix, deviceDict[dev1], cIndex+idx2, mosIndex, deviceDict[dev1]['Inverse'])
    ...
    print('[Info] circuit {} is finished'.format(circuit))
